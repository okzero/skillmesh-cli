...
> 多 Agent × 多机器 Skill 对等同步工具。一次安装，软链接分发到所有 Agent；hub 目录经任意同步后端（iCloud / Dropbox / Google Drive / Syncthing / 坚果云 / 百度网盘 / 夸克网盘 / 阿里云盘 / git）跨机同步；所有节点对等，无中心写者。
...
- **单机多 Agent**：装一次，软链接分发到所有声明接收它的 Agent 目录
...
| **directory / file layout** | 目录级软链（codex/claude）或文件级软链（cursor .mdc） |
| **跨平台** | macOS（launchd）+ Linux（systemd），同代码 |
...
| 分发方式 | symlink 优先 + copy fallback | copy only | symlink | symlink |
...
要求：Python 3.9+（macOS / Linux 自带）。TOML config 需 3.11+，3.9/3.10 用 JSON config。
...
macOS 用 launchd，Linux 用 systemd user unit。
